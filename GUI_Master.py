...
class DisGUI():
    def __init__(self, root, serial, data):
        ...
        self.frames = []        # a list of all the frames that will be created
        self.framesCol = 0      # coloumn inital is 0
        self.framesRow = 4      # four (first three rows are used by the serial, communication, and root GUI)
        self.totalframes = 0
    ...
    def AddMasterFrame(self):
        self.frames.append(LabelFrame(self.root, text=f"Display Manager-{len(self.frames)+1}",pady=5, padx=5, bg="white"))
    ...
    def AddGraph(self):
        ...
        self.figs[self.totalframes][2].get_tk_widget().grid(column=1, row=0, columnspan=4, rowspan=17,  sticky=N)
    ...
    def ChannelFunc(self, Frame, ChannelFrameNumber):
        ...
        drop_ch = OptionMenu(Frame, self.FunVar[ChannelFrameNumber][len(self.OptionVar[ChannelFrameNumber])-1], *bds)
        drop_ch.config(width=5)
